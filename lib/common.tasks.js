 ...
 */
function buildLetterTasks(letterName, smtpOptions = null) {
  ...
  return {
    ...
    serve,
    sendTestMail,
    setSmtpOptions: smtp => (options.smtp = smtp)
  };
  ...
  function compilePug() {
    return gulp
      .src(options.pug.src)
      .pipe(pug({ data: { code: '${options.code}' } }))
  ...
  function sendTestMail() {
    return gulp
      ...
      .pipe(sendMail(letterName, options.smtp));
  }
  ...
  function serve() {
    ...
    gulp.watch([options.pug.src], gulp.series([compilePug, reload]));
    gulp.watch([options.sass.src], gulp.series([compileSass, reload]));
  ...
  // gulp.task(
  //   letterName + 'watch',
  //   gulp.series([letterName + 'browser-sync'], function() {
  //     gulp.watch(options.src.sass, [letterName + 'sass']); // Наблюдение за sass файлами в папке sass
  //     gulp.watch(options.src.pug, [letterName + 'pug']);
  //     gulp.watch(options.src.html, browserSync.reload); // Наблюдение за HTML файлами в корне проекта
  //   })
  // );
}
