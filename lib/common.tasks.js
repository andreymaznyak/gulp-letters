...
const pugCompilation = require('./pug-compilation.task.js');
...
function buildLetterTasks(letterName, extOptions = null) {
  ...
  const options = {
    sass: {
      ...
      dist: 'dev/css/' + (letterName.split('/').length === 1 ? '' : letterName.split('/').slice(0, -1).join('/')) + '/' // sass кладем в dev/css
    },
    pug: {
    ...
    html: {
      src: 'dev/' + (letterName.split('/').length === 1 ? letterName : letterName.split('/').pop(-1)) + '.html',
  ...
  function compilePug() {
    ...
    return gulp
      .src(options.pug.src)
      .pipe(pugCompilation({ data, client: false, isBuild: taskName === 'build' }))
  ...
  function copyCssToDist() {
    const letterPath = letterName.split('/');
    return gulp
        .src([options.sass.dist + (letterPath.length === 1 ? letterName : letterPath[letterPath.length - 1]) + '.css'])
  ...
  function putInlineStylesToHtml() {
    ...
    return gulp
      ...
      .pipe(extReplace(taskName === 'build' ? '.js' : '.html'))
  ...
  function sendTestMail() {
    return gulp
      .src(options.html.dist + letterName + '.html')
