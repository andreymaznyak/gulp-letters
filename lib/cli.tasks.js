...
function walkSync(dir, filelist) {
  ...
  files.forEach(function(file) {
    const potentialDir = path.join(dir, file);
    if (fs.statSync(potentialDir).isDirectory()) {
      _filelist = walkSync(potentialDir, _filelist, );
    }
    else {
    ...
    gulp.task('build', async done => {
      ...
      try {
        ...
        if (e instanceof EmptyNameError) {
          const sercPath = 'src';
          const files = walkSync('src').map(filePath => filePath.substr(sercPath.length + 1));
          letters = files
            .filter(filePath => {
              const filePathParts = filePath.split('/');
              if (filePathParts.length === 1) {
                return !!filePath.match(/^[^_].*\.pug$/);
              } else {
                return !!filePathParts[filePathParts.length - 1].match(/^[^_].*\.pug$/);
              }
            })
